# 🧑🏻‍💻 Kim Kwangrae

안정적인 서비스와 깔끔한 구조를 고민하는 백엔드 개발자 김광래입니다.

서비스 흐름을 이해하고 안정적인 서버 구조를 설계하는 데 관심이 많습니다.
확장성과 유지보수를 고려한 코드, 문서 그리고 요구사항이 반영되어 실제로 운영 가능한 시스템을 만드는 것을 목표로 합니다.

확장 가능한 설계를 통해 신뢰할 수 있는 서비스를 만드는 것을 추구합니다.

## 🔥 Activities

<!-- <table>
<tr>
  <th>BeakJoon</th>
  <th>Programmers</th>
</tr>
<tr>
  <td>
    <img src="https://mazassumnida.wtf/api/v2/generate_badge?boj=kkr010128" width="300" style="display:block;"/>
  </td>
  <td>
    <img src="https://raw.githubusercontent.com/kkr010128/Programmers_Badge_Generator/main/result/result.svg" width="350"/>
  </td>
</tr>
</table> -->

| 프로젝트 명 | 기간 | 설명 | 주요 기술 | 형태 |
| -------- | --- | --- | ------- | --- |
| [학점 가방](https://github.com/Dongguk-unibag) | 2024.07 ~ 2025.5 | 학생 편의 개선 목적의 동국대학교 학사/생활 통합 서비스 | Node.js, Flutter, MySQL, Redis | 팀 프로젝트 |
| [함께, 경주](https://github.com/kkr010128/2024hackathon_200_OK) | 2024.11 | 카드 형태의 숏폼을 이용한 경주시와 시민의 정책 소통 플랫폼 | React.js, MySQL | 팀 프로젝트</br>(해커톤) |
| [POLY DASH](https://github.com/kkr010128/Poly-DashBoard) | 2024.4 | DRM 시스템의 관리용 웹 대시보드 | Flask, Sqlite | 개인 프로젝트 |
| [POLYGON](https://github.com/kkr010128/Polygon-License-System) | 2020.04 ~ | 3D 오브젝트 디지털 상품에 대한 사용권 및 라이센스 관리 시스템 | Flask, Sqlite, Discord.py | 개인 프로젝트</br>(서비스 진행중) |

## 🏆 Awards
| 대회명 | 기관 | 작품 설명 | 수상 내용 | 수상일 |
|----------|---------|-----|-----|-----|
| <p align="center"> 2024 경주 지역문제 해결 해커톤 </p> | <p align="center"> 경주시 </p> | 경주시 정책 소통 플랫폼 | <p align="center"> 대상 | <p align="center"> 2024.12.01 </p> |
| <p align="center"> 제 4회 링크 창업 아이템 경진대회</p> | <p align="center"> 동국대학교 </p> | 재학생을 위한 택시 합승 매칭 웹앱 | <p align="center"> 장려상 | <p align="center"> 2024.06.24</p> |

## 📰 Article
[동국대 WISE캠퍼스 2024 경주 지역문제해결형 해커톤 경진대회 개최](https://www.veritas-a.com/news/articleView.html?idxno=534327#google_vignette)

<!-- ## 📜 Certifications

- **SQLD** : 2024.09.20 취득
- **DAsP** : 2024.10.25 취득
- **ADsP** : 2024.11.29 취득
- **PCCE Lv4** : 2025.04.20 취득
- **정보처리기사** : 2025.09.11 취득 -->

## 🛠 Tech Stack

- **Language** : Python, Java, JavaScript, Lua
- **DevOps** : Docker
- **Front-End** : Flutter, React
- **Back-End** : Spring-boot, Spring Security, JPA
- **Database** : MySQL, Redis, Sqlite, MongoDB
- **ETC** : Git, Notion, Ubuntu, Figma
